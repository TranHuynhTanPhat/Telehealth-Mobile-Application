...
import 'package:healthline/data/api/repositories/file_repository.dart';
...
class HealthInfoCubit extends HydratedCubit<HealthInfoState> {
  ...
  Future<void> fetchMedicalRecord() async {
    ...
    try {
      ...
      if (userResponses.indexWhere((element) => element.isMainProfile!) != -1) {
  ...
  Future<void> addSubUser(String avatar, String fullName, String birthday,
    ...
    try {
      ...
      emit(AddUserFailure(state.subUsers, state.currentUser,
          er.response!.data['message'].toString()));
    }
  }
  ...
  Future<void> updateUser(File? avatar, String fullName, String birthday,
    ...
    try {
      ...
      emit(UpdateUserFailure(state.subUsers, state.currentUser,
          er.response!.data['message'].toString()));
    }
  }
